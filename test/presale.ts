...
describe('Presale Contract', function () {
    ...
    describe('Whitelisting', function () {
        ...
        it('should revert if non-owner tries to whitelist addresses', async function () {
            await expect(
                presale.connect(investor1).whitelistAddresses([investor1.address])
        ...
        it('should revert if non-owner tries to remove addresses from the whitelist', async function () {
            await expect(
                presale.connect(investor1).removeWhitelistAddresses([investor1.address])
            ).to.be.revertedWithCustomError(presale, 'NotOwner');
        });
    });
    ...
    describe('Token Deposit', function () {
        ...
        it('should not allow non-owners to deposit tokens', async function () {
            await expect(presale.connect(investor1).depositTokens()).to.be.revertedWithCustomError(presale, 'NotOwner');
        });
    ...
    describe('Contributions with Bonuses', function () {
        ...
        it('should reject contributions when the funding period is over', async function () {
            ...
            await expect(
                presale.connect(investor1).contribute({ value: ethers.utils.parseEther('1') })
            ).to.be.to.be.revertedWithCustomError(presale, 'NotInFundingPeriod');
        });
    });
    ...
    describe('Whitelist contributions', function () {
        ...
        it('should reject contributions of non whitelisted during whitelist period', async function () {
            const contributionAmount = ethers.utils.parseEther('10'); // 10 ETH
            await expect(presale.connect(investor1).contribute({ value: contributionAmount })).to.be.revertedWithCustomError(
                presale,
                'NotWhitelisted'
            );
        });
